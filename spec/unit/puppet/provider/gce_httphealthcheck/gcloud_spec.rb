...
describe Puppet::Type.type(:gce_httphealthcheck).provider(:gcloud) do
  let(:resource) { Puppet::Type.type(:gce_httphealthcheck).new(:name => 'httphealthcheck1') }
  let(:provider) { resource.provider }


  let(:params) { ['compute', 'http-health-checks', 'httphealthcheck1'] }
  ...
  context "with an invalid resource" do
    let(:resource) { Puppet::Type.type(:gce_httphealthcheck).new(:name => 'invalid-httphealthcheck') }
    let(:provider) { resource.provider }
